...
def _build_generator(args, ebird_codes=None, segments=None):
    ...
    if args.model_type == "ecogen":
        from generator.ecogen_generator import EcogenGenerator

        filtered = [s for s in segments if s["ebird_code"] in ebird_codes]
        return EcogenGenerator(
            model_path=args.checkpoint,
            segments=filtered,
            device=args.device,
            augmentation=args.augmentation,
            ratio=args.ratio,
            latent_stats_path=getattr(args, "latent_stats", None),
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["audiogen", "audiogen_text", "llama", "ecogen"],
    )
    parser.add_argument(
        "--stage", type=str, required=True, choices=["pretrain", "finetune", "baseline"]
    )
    ...
    if args.model_type != "audiogen_text" and args.checkpoint is None:
        parser.error(
            "--checkpoint is required for model types other than audiogen_text"
        )

    if args.temperature is None:
        args.temperature = (
            AG_GEN_TEMPERATURE
            if args.model_type in ("audiogen", "audiogen_text")
    ...
    output_dir = (
        Path(args.output)
        if args.output
        else _default_output_dir(args.model_type, args.stage)
    )
    ...
    logger.info(
        "found %d segments spanning %d unique classes",
        len(segments),
        len(test_ebird_codes),
    )

    if args.model_type == "audiogen_text":
        logger.info(
            "loading pretrained AudioGen with text conditioning (template=%s)",
            args.prompt_template,
        )
    elif args.model_type == "ecogen":
        logger.info(
            "loading ecogen VQ-VAE from %s (aug=%s, ratio=%.2f)",
            args.checkpoint,
            args.augmentation,
            args.ratio,
        )
    ...
    t0 = time.time()
    generator = _build_generator(args, ebird_codes=test_ebird_codes, segments=segments)
    logger.info(
        "generator loaded in %.1fs (%d known classes)",
        time.time() - t0,
        len(generator.ebird_to_id),
    )
    ...
    if use_reranker:
        ...
        classes = sorted(
            c for c in test_ebird_codes if c in generator_codes and c in disc_codes
        )
        n_skipped = len(test_ebird_codes) - len(classes)
        if n_skipped:
            logger.warning(
                "skipping %d test classes not covered by generator+discriminator",
                n_skipped,
            )
    else:
        ...
        if n_skipped:
            logger.warning(
                "skipping %d test classes not in generator vocabulary", n_skipped
            )
    ...
    for ebird_code in class_bar:
        ...
        class_bar.set_postfix(
            done=total_generated,
            failed=total_failed,
            current=ebird_code,
        )
        if class_failed:
            logger.warning(
                "%s: %d/%d samples failed",
                ebird_code,
                class_failed,
                args.n_per_class,
            )
    ...
    test_counts = Counter(
        seg["ebird_code"] for seg in segments if seg["ebird_code"] in set(classes)
    )
    metadata = {
    ...
    if args.model_type == "audiogen_text":
        ...
        metadata["prompts"] = {
            code: generator._prompts[code]
            for code in classes
            if code in generator._prompts
        }
